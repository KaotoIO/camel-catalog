...
class CamelYamlDslSchemaProcessorTest {
    ...
    @Test
    void testDefinitionsAreNotEmpty() throws Exception {
        var dataFormatMap = processor.getDataFormats();
        var languageMap = processor.getLanguages();
        var loadBalancerMap = processor.getLoadBalancers();

        dataFormatMap.values().forEach(this::assertDefinitionsAreNotEmpty);
        languageMap.values().forEach(this::assertDefinitionsAreNotEmpty);
        loadBalancerMap.values().forEach(this::assertDefinitionsAreNotEmpty);
    }

    private void assertDefinitionsAreNotEmpty(ObjectNode schema) {
